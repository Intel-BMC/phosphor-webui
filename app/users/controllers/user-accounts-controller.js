...
window.angular && (function(angular) {
  ...
  angular.module('app.users').controller('userAccountsController', [
    '$scope', 'APIUtils', '$filter', 'toastService', '$uibModal',
    function($scope, APIUtils, $filter, toastService, $uibModal) {
      $scope.loading;
      ...
      $scope.tableModel.header = ['Username', 'Privilege', 'Status']
      $scope.tableModel.actions = ['Edit', 'Delete'];

      $scope.saveUser = function() {
        //    const newUser = user ? false : true;
        //    const originalUsername = user ? angular.copy(user.UserName) :
        //    null; const template = require('./user-accounts-modal-user.html');
        this.user = {};
        ...
        this.automaticLockout =
            mapLockoutMethod($scope.accountSettings.AccountLockoutDuration);
        ...
        if (form.$valid) {
          ...
          const role =
              form.privilege.$pristine ? null : form.privilege.$modelValue;
          const enabled =
              (form.accountStatus.$pristine && form.accountStatus1.$pristine) ?
              null :
              form.accountStatus.$modelValue;
          const locked =
              (form.lock && form.lock.$dirty) ? form.lock.$modelValue : null;
          if (!newUser) {
            updateUser(
       ...
       */
      $scope.onClickDeleteUser = (user) => {
        initRemoveModal(user);
      };

      /**
       * Callback when 'Reset password' button clicked
       */
      $scope.onClickModifyUser = (user) => {
        initUserModal(user);
      };
